...
Google x Kaggle 5-Day Agentic AI Intensive Capstone 2026
Built by Aiza Fatima | github.com/AIZUFA/GlowAI-Skincare-Agent
"""
...
Be warm, professional and specific to what you see."""}
        ...
        return f"⚠️ Could not analyze image: {str(e)}"
...
async def init_session():
    await session_service.create_session(
        app_name="GlowAI", user_id="user_001", session_id="session_001"
    )
    await session_service.create_session(
        app_name="GlowAI", user_id="user_001", session_id="session_002"
    )
...
async def skincare_chat(message, image, history):
    ...
    try:
        ...
        history = history + [
            {"role": "user", "content": message or "📸 Skin photo uploaded"},
        ...
        history = history + [
            ...
            {"role": "assistant", "content": f"✨ GlowAI is taking a short break. Please try again in a moment! 🌸\n\nError: {str(e)}"}
        ]
        return "", None, history
...
async def makeup_chat(message, image, history):
    ...
    try:
        ...
        history = history + [
            {"role": "user", "content": message or "📸 Face photo uploaded"},
            {"role": "assistant", "content": response_text}
        ]
        return "", None, history
...
with gr.Blocks(css=css, title="GlowAI ✨") as demo:
    ...
    with gr.Tabs():
        ...
        with gr.Tab("🧴 Skincare"):
            ...
            skincare_chatbot = gr.Chatbot(label="", height=420, show_label=False, type="messages")
            with gr.Row():
        ...
        with gr.Tab("💄 Makeup"):
            ...
            makeup_chatbot = gr.Chatbot(label="", height=420, show_label=False, type="messages")
            with gr.Row():
